    ...
    <div>
      <div class="mb-6">
        ...
        <div class="tabs tabs-boxed">
          ...
          <button
            class="tab"
            :class="{ 'tab-active': activeTab === 'speedtest' }"
          ...
          <ul class="menu bg-base-100 w-full">
            ...
            >
              <a
                ...
                class="hover:bg-base-200 focus:bg-base-300"
                :class="[
        ...
        <div class="flex-1">
          ...
          <div v-if="selectedProvider" class="space-y-6">
            ...
            <div>
              ...
              <ul
                class="menu bg-base-100 w-full [&>li:not(:last-child)]:border-b border-base-200"
          ...
          <!-- 底部按钮 -->
          <div
            class="flex justify-end gap-4 mt-6 border-t border-base-300 pt-6"
          >
            ...
            <div class="form-control">
              <label class="label">
                <span class="label-text">{{
                  t("apiSelector.modelDescription")
                }}</span>
              </label>
              <textarea
      ...
      <div v-show="activeTab === 'speedtest'" class="mt-4">
        <div class="flex justify-between items-center mb-6">
          ...
          >
            {{
              speedTesting
                ? t("apiSelector.speedTesting")
                : t("apiSelector.speedTest")
            }}
          </button>
        </div>
          ...
          <table class="table w-full">
            ...
            <tbody>
              <tr v-for="result in speedResults" :key="result.provider">
                ...
                <td v-else class="text-error">
                  {{ result.error || t("apiSelector.speedTestNoKey") }}
                </td>
              </tr>
            </tbody>
          </table>
...
const handleConnectionTest = async () => {
  ...
  try {
    ...
    if (response.success) {
      showToast(
        t
          .value("apiSelector.connectionSuccess")
          .replace("{time}", response.responseTime),
        "success"
      );
    } else {
      showToast(
        t
          .value("apiSelector.connectionError")
    ...
    showToast(
      t.value("apiSelector.connectionError").replace("{error}", error.message),
      "error"
    );
  } finally {
...
const saveModel = async () => {
  try {
    ...
    showToast(
      t.value("apiSelector.modelSaveError").replace("{error}", error.message),
      "error"
    );
  }
};
...
const deleteModel = async (model) => {
  ...
  try {
    ...
    showToast(
      t.value("apiSelector.modelDeleteError").replace("{error}", error.message),
      "error"
    );
  }
};
...
const save = async () => {
  ...
  try {
    ...
    showToast(
      t.value("apiSelector.saveError").replace("{error}", error.message),
      "error"
    );
  }
};
