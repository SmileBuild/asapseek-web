<template>
    <main class="flex-1 flex flex-col h-screen overflow-hidden bg-base-100">
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-4" ref="messagesContainer">
        ...
        <div v-if="messages.length === 0" 
          ...
          <div class="max-w-2xl w-full bg-base-200 rounded-xl p-8 space-y-6">
            <!-- Welcome Header -->
            <h2 class="text-2xl font-semibold text-base-content">{{ t('chat.emptyStateTitle') }}</h2>
            <p class="text-lg text-base-content/80">{{ t('chat.emptyStateDescription') }}</p>

            <!-- Tips List -->
            <div class="space-y-4">
              <ul class="space-y-3">
                <li v-for="(tip, index) in t('chat.emptyStateTips')" 
                    :key="index"
        ...
        <div v-for="message in messages" :key="message.id" 
          ...
          <div class="flex gap-3 items-start max-w-3xl w-full" :class="[message.sender === 'user' ? 'flex-row-reverse' : 'flex-row']">
            ...
            <div class="w-8 h-8 shrink-0 text-base-content">
              <UserCircleIcon v-if="message.sender === 'user'" class="w-full h-full" />
            ...
            <div class="flex-1 group relative">
              ...
              <div class="flex items-center gap-2 mb-1" 
                ...
                <span class="font-medium text-base-content">{{ messageTypes[message.sender].name }}</span>
                <span class="text-sm text-base-content/70">{{ formatTime(message.timestamp) }}</span>
                <span v-if="message.usage" class="text-sm text-base-content/60">
                  ({{ message.usage.total_tokens }} tokens: {{ message.usage.prompt_tokens }} + {{ message.usage.completion_tokens }} completion)
                </span>
              </div>
              ...
              <div :class="[
                ...
                <div v-if="message.sender === 'assistant'">
                  ...
                  <div v-if="message.reasoning_content" 
                    ...
                    <div class="text-sm font-medium text-base-content/70 mb-2">Reasoning</div>
                    <div class="text-sm bg-base-200 p-3 rounded text-base-content">
        ...
        <div v-if="isStreaming" class="flex justify-start">
          <div class="flex gap-3 items-start max-w-3xl w-full">
            ...
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-1">
                <span class="font-medium text-base-content">{{ messageTypes.assistant.name }}</span>
              </div>
              <div class="p-4 rounded-lg relative markdown-body bg-base-200 text-base-content shadow-sm">
                <!-- Streaming Reasoning Content -->
                <div v-if="streamingReasoningContent" class="mb-4 pb-4 border-b border-base-300">
                  <div class="text-sm font-medium text-base-content/70 mb-2">Reasoning</div>
        ...
        <div v-if="isLoading" class="flex justify-start">
          <div class="max-w-3xl p-4 rounded-lg bg-base-200 text-base-content">
            <div class="flex gap-2">
              ...
              <div class="w-2 h-2 bg-green-500/50  rounded-full animate-bounce [animation-delay:0.2s]"></div>
              <div class="w-2 h-2 bg-green-500/50  rounded-full animate-bounce [animation-delay:0.4s]"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="p-1 flex gap-3 justify-center">
        <button class="btn btn-sm btn-outline tooltip" :data-tip="t('chat.uploadDocument')">
          <DocumentArrowUpIcon class="h-4 w-4" />
        </button>
        <button class="btn btn-sm btn-outline tooltip" :data-tip="t('chat.onlineSearch')">
          <GlobeAltIcon class="h-4 w-4" />
        </button>
        <button class="btn btn-sm btn-outline tooltip" :data-tip="t('chat.clearChat')"
          @click="$emit('clear-chat')">
          <TrashIcon class="h-4 w-4" />
        </button>
        <button class="btn btn-sm btn-outline tooltip" :data-tip="t('chat.knowledgeBase')">
      ...
      <div v-if="showExportSuccessAlert"
           class="fixed inset-0 flex items-center justify-center z-50">
        <div class="alert alert-success shadow-lg">
          <CheckCircleIcon class="w-5 h-5" />
